...
class Excel_upload extends CI_Controller {
        ...
        try {
            ...
            if ($file_extension === 'csv') {
                ...
                if ($xlsx = \Shuchkin\SimpleXLSX::parse($file_path)) {
                    ...
                    log_message('debug', '엑셀 파일 읽기 완료. 총 ' . count($rows) . '행');
                    if (!empty($rows)) {
                        log_message('debug', '첫 번째 행: ' . implode(', ', $rows[0]));
                    }
    ...
    private function process_excel_data($excel_data)
    {
        ...
        // 헤더 행 찾기 (첫 번째 행이 비어있을 수 있음)
        $headers = array();
        $header_row_index = 0;
        
        // 최대 10행까지 확인하여 유효한 헤더 찾기
        for ($i = 0; $i < min(10, count($excel_data)); $i++) {
            $potential_headers = array_map('trim', $excel_data[$i]);
            
            // 의료기관 관련 키워드가 포함된 행을 헤더로 판단
            $medical_keywords = ['기관', '병원', '의료', '주소', '전화', '연락', '소재지', '요양기관', '암호화', '종별', '시도'];
            $keyword_matches = 0;
            
            foreach ($potential_headers as $header) {
            ...
            // 조건: 비어있지 않은 컬럼이 5개 이상이고, 의료기관 관련 키워드가 2개 이상
            if ($non_empty_count >= 5 && $keyword_matches >= 2) {
                $headers = $potential_headers;
                $header_row_index = $i;
                log_message('debug', "헤더 후보 발견 - 행 {$i}: 비어있지 않은 컬럼 {$non_empty_count}개, 키워드 매치 {$keyword_matches}개");
                break;
            }
        }
        
        // 헤더를 찾지 못한 경우, 가장 많은 비어있지 않은 컬럼을 가진 행을 선택
        if (empty($headers)) {
            $max_columns = 0;
            $best_row_index = 0;
            ...
            for ($i = 0; $i < min(10, count($excel_data)); $i++) {
                ...
                if ($non_empty_count > $max_columns) {
                    $max_columns = $non_empty_count;
                    $best_row_index = $i;
                    $headers = $potential_headers;
                }
            }
            ...
            log_message('debug', "키워드 매치 실패, 최대 컬럼 수 기준으로 선택 - 행 {$best_row_index}: {$max_columns}개 컬럼");
        }
        
        // 여전히 헤더를 찾지 못한 경우
        if (empty($headers) || count(array_filter($headers, function($h) { return !empty($h); })) < 3) {
            $debug_info = "파일 분석 결과:\n";
            for ($i = 0; $i < min(5, count($excel_data)); $i++) {
                $debug_info .= "행 {$i}: " . implode(' | ', array_map('trim', $excel_data[$i])) . "\n";
            }
            throw new Exception("유효한 헤더를 찾을 수 없습니다.\n\n" . $debug_info . "\n엑셀 파일의 구조를 확인해주세요.");
        }
        
        log_message('debug', "최종 헤더 행 인덱스: {$header_row_index}, 헤더: " . implode(' | ', $headers));
        ...
        // 헤더 매핑 결과 로깅
        log_message('debug', '매핑된 헤더 인덱스: ' . json_encode($header_indices));
        
        // 필수 필드 확인 전에 대체 매핑 시도
        $required_fields = array('institution_name', 'address', 'phone');
        $missing_fields = array();
        
        // 대체 매핑 시도 (인덱스 기반)
        if (!isset($header_indices['institution_name']) && count($headers) >= 2) {
            // 두 번째 컬럼을 기관명으로 가정
            $header_indices['institution_name'] = 1;
            log_message('debug', '기관명을 두 번째 컬럼(인덱스 1)으로 가정');
        }
        
        if (!isset($header_indices['address']) && count($headers) >= 11) {
            // 11번째 컬럼을 주소로 가정 (일반적인 의료기관 데이터 구조)
            $header_indices['address'] = 10;
            log_message('debug', '주소를 11번째 컬럼(인덱스 10)으로 가정');
        }
        
        if (!isset($header_indices['phone']) && count($headers) >= 12) {
            // 12번째 컬럼을 전화번호로 가정
            $header_indices['phone'] = 11;
            log_message('debug', '전화번호를 12번째 컬럼(인덱스 11)으로 가정');
        }
        ...
        if (!empty($missing_fields)) {
            $error_msg = "필수 컬럼이 누락되었습니다: " . implode(', ', $missing_fields) . 
                        "\n\n=== 파일 분석 정보 ===\n" .
                        "총 컬럼 수: " . count($headers) . "\n" .
                        "헤더 행 위치: " . ($header_row_index + 1) . "행\n" .
                        "업로드된 파일의 헤더:\n";
            
            for ($i = 0; $i < count($headers); $i++) {
                $header_value = empty(trim($headers[$i])) ? '[빈값]' : trim($headers[$i]);
                $error_msg .= "  컬럼 " . ($i + 1) . ": " . $header_value . "\n";
            }
            
            $error_msg .= "\n=== 지원되는 헤더 예시 ===\n";
            $error_msg .= "• 기관명: 요양기관명, 기관명, 병원명, 의료기관명\n";
            $error_msg .= "• 주소: 주소, 소재지\n";
            $error_msg .= "• 전화번호: 전화번호, 연락처, TEL\n";
            $error_msg .= "\n※ 헤더가 올바르지 않은 경우 http://localhost/excel_debug 에서 파일 구조를 확인해보세요.";
            
            throw new Exception($error_msg);
        }

        // 데이터 행 처리 (헤더 행 다음부터 시작)
        for ($i = $header_row_index + 1; $i < count($excel_data); $i++) {
            try {
                $row = $excel_data[$i];
